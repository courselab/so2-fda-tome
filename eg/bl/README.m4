...
2) Take some time to understand the program source.
   ...
   Still on 'stage1.c ',the function 'load_kernel()', if successfull, uses the
   BIOS disk services to read a few more disk sectors (containing the kernel)
   from the disk and load them into RAM. 
